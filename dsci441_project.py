...
ir_df = pd.read_csv(csv_path)

print(ir_df.head())
print(ir_df.tail())

# select same range of dates of housing data and only the overall unemployment rate
ir_df = ir_df.iloc[576:576+291][['unrate','date']]
ir_df = ir_df.reset_index(drop=True)
...
full_df['Unemployment Rate'] = ir_df['unrate']
...
ax1.plot(full_df['Year-Month'], full_df['ZHI'], color='blue', label='ZHI')
ax1.set_xlabel('Year')
...
ax2.plot(ir_df['TimeIndex'], ir_df['unrate'], color='red', label='Unemployment Rate')
...
X_train = train[['Year', 'Month', 'TimeIndex', 'Unemployment Rate']]
y_train = train['ZHVI']
...
# Prediction test
X_test = test[['Year', 'Month', 'TimeIndex','Unemployment Rate']]
